...
def submit_sbatch(job_kwargs, gpu=False):
    ...
    with open('{tempfile_dir}/{name}.sh'.format(**job_kwargs), 'w+') as fh:
        ...
        fh.writelines('#SBATCH --output={log_path}/{name}_%N_%j.out\n'.format(**job_kwargs))
        fh.writelines('#SBATCH --error={log_path}/{name}_%N_%j.err\n'.format(**job_kwargs))
