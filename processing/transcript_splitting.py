...
def process_chunk(df):
    ...
    Returns:
        MultiIndexed dataframe monotonicly sorted.
    """
...
def relative(df, section):
    """
    Removes y/x min -> relative coordinates.

    Args:
        df: DataFrame
        section: string of the section working on.
...
def save_section(section):
    ...
    print('relative')
    # save thingy
    csv_path = Path(processed / '{0}/transcripts/relative.csv'.format(seciton))
    csv_path.mkdir(parents=True, exist_ok=True)
    relative_px_filtered_sub_results_df.to_csv(processed / '{0}/transcripts/relative.csv'.format(section), index=False)
    # compress for ProSeg
    gz_path = Path(processed / '{0}/transcripts/relative.csv.gz'.format(section))
    gz_path.mkdir(parents=True, exist_ok=True)
    with open(csv_path, 'rb') as df, gzip.open(gz_path, 'w') as df_gz:
        df_gz.writelines(df)
    print('saved restults')
...
if __name__ == '__main__':
    ...
    with mp.Pool(processes=mp.cpu_count()-1) as pool:
        with pd.read_csv(data / 'transcripts.csv.gz', compression = 'infer', dtype=dtype_dict, chunksize = 20000) as reader:
    ...
    # save the sections
    # with mp.Pool(processes=mp.cpu_count()-1) as pool:
    #     pool.imap_unordered(save_section, [section for section,coordinates in sections_dict.items()])
    #     pool.close()
    #     pool.join()
    save_section('0')