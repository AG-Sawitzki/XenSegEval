...
def pixelate(df, pixelsize):
    ...
    """
    df['y_location'] = (df['y_location'] 
                        / pixelsize[0]).round(0).astype(np.int64)
    df['x_location'] = (df['x_location'] 
                        / pixelsize[0]).round(0).astype(np.int64)
	df['z_location'] = (df['z_location'] 
                        / pixelsize[1]).round(0).astype(np.int64)
